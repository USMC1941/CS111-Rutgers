...
public class Picasso {
    ...
    public static void drawLineDown(int length) {
        drawing.drawLine(penx, peny, penx, peny - length * SCALE);
    }

    public static void drawLineRight(int length) {
        drawing.drawLine(penx, peny, penx + length * SCALE, peny);
    }
...
class Drawing extends JFrame {
    ...
    public void drawCircle(int centerx, int centery, int radius) {
        ...
        for (x = x1; x <= x2; x++) {
            double sqrt = Math.sqrt(radius * radius - Math.pow(x - centerx, 2));
            y = centery + sqrt;
            drawDot(x, y);
            y = centery - sqrt;
            drawDot(x, y);
        ...
        for (y = y1; y <= y2; y++) {
            double sqrt = Math.sqrt(radius * radius - Math.pow(y - centery, 2));
            x = centerx + sqrt;
            drawDot(x, y);
            x = centerx - sqrt;
            drawDot(x, y);
    ...
    public void drawSolidCircle(double centerx, double centery, double radius) {
        drawSolidCircle(
                (int) Math.round(centerx), (int) Math.round(centery), (int) Math.round(radius));
    }
