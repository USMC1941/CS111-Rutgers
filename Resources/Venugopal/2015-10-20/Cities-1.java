public class Cities {
	...
	public static String getCity() {
		String city;
		String prompt = "Enter city name (letters and spaces ONLY, " + "at least one character), " + "'quit' to stop: ";
		do {
	...
	// adds the given city to the master cities string
	// and returns the updated cities string
	public static String add(String cities, String city) {
		int start = 0;
		int pos   = cities.indexOf(';');
		while (pos != -1) {    // pos will be -1 if no semicolon is found in the string
			String nextCity = cities.substring(start, pos);    // city name that is in the range start..pos-1

			int c = city.compareToIgnoreCase(nextCity);
			if (c == 0) {
				return cities;  // city already in list, nothing to do, return cities without change
			}
			...
			// c > 0, keep going
			start = pos + 1;
			pos = cities.indexOf(';', start);   // look for ';' starting at "start" index, not beginning of string
