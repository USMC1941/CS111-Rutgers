/**
 * Definition of calendar object. A calendar keeps a chronological list of events/appointments, and
 * allows adds, deletes, and modifications of events
 */
public class Calendar {
    private String username;
    private Event[] events;

    /** number of actual events in the array */
    private int size;
    ...
    /**
     * adds a new event in chronological order. if this conflicts with an existing event, a warning
     * message is printed, and the user has the option to go ahead with the add or cancel it. method
     * returns true if the event was added, false if not
     */
    public boolean addEvent(Event event) {
        // full house, can't add
    ...
    private boolean resolveConflict(int i, Event event) {
        ...
        while (i < size) {
            ...
            // insert at i after moving stuff over
            if (eventSeconds <= iSeconds) {
                for (int j = size - 1; j >= i; j--) {
                    events[j + 1] = events[j];
                }
                events[i] = event;
    ...
    /** deletes the given event if event does not exist, returns false, otherwise true */
    ...
    /**
     * changes original event to new event if original event is not found, returns false, otherwise
     * true
     */
